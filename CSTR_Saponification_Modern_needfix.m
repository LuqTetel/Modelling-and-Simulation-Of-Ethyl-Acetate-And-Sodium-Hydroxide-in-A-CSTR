classdef CSTR_Saponification_Modern_needfix < matlab.apps.AppBase
...
PlotContextMenu             matlab.ui.container.ContextMenu
    ...
    UncertaintyButton           matlab.ui.control.Button
    ResidualsButton             matlab.ui.control.Button
...
UltraTemperatureConverged  logical = false
  ...
  % NEW: Enhanced functionality properties (ADD THESE)
    UncertaintyResults          struct
    DiagnosticsResults          struct
    ValidationResults           struct
    OperationCancelled         logical = false
    
    % NEW: Cancellable dialog components (ADD THESE)
    CancellableDialog           matlab.ui.Figure
    CancellableMessage          matlab.ui.control.Label
    CancellableProgress         matlab.ui.control.Slider
...
function [ss_time, ss_value] = detectSteadyStateHelper(app, time_data, conversion_data)
    try
        ...
        function setupDesignSystem(app)
            ...
            % NEW: Enhanced buttons styling (ADD THESE)
if isfield(app, 'UncertaintyButton') && ~isempty(app.UncertaintyButton)
    app.UncertaintyButton.BackgroundColor = [138 43 226]/255; % Blue Violet
    app.UncertaintyButton.FontColor = app.SurfaceColor;
end

if isfield(app, 'ResidualsButton') && ~isempty(app.ResidualsButton)
    app.ResidualsButton.BackgroundColor = [255 140 0]/255; % Dark Orange
    app.ResidualsButton.FontColor = app.SurfaceColor;
end
...
% NEW: Initialize enhanced results (ADD THESE)
app.UncertaintyResults = struct();
app.DiagnosticsResults = struct();
app.ValidationResults = struct();
...
app.updateConversionInfo();
    ...
    try
        ...
        % Initialize threshold values with safe defaults if not set
        manualReactantThresh = 0.5;
        manualProductThresh = 0.5;
        manualTempThresh = 0.5;
        optimizedReactantThresh = 0.5;
        optimizedProductThresh = 0.5;
        optimizedTempThresh = 0.5;
        quickSimReactantThresh = 0.5;
        quickSimProductThresh = 0.5;
        quickSimTempThresh = 0.5;
        
        % Safely get threshold values if they exist
        if isprop(app, 'ManualReactantThreshold') && ~isempty(app.ManualReactantThreshold)
            manualReactantThresh = app.ManualReactantThreshold * 100;
        end
        if isprop(app, 'ManualProductThreshold') && ~isempty(app.ManualProductThreshold)
            manualProductThresh = app.ManualProductThreshold * 100;
        end
        if isprop(app, 'ManualTemperatureThreshold') && ~isempty(app.ManualTemperatureThreshold)
            manualTempThresh = app.ManualTemperatureThreshold * 100;
        end
        
        if isprop(app, 'OptimizedReactantThreshold') && ~isempty(app.OptimizedReactantThreshold)
            optimizedReactantThresh = app.OptimizedReactantThreshold * 100;
        end
        if isprop(app, 'OptimizedProductThreshold') && ~isempty(app.OptimizedProductThreshold)
            optimizedProductThresh = app.OptimizedProductThreshold * 100;
        end
        if isprop(app, 'OptimizedTemperatureThreshold') && ~isempty(app.OptimizedTemperatureThreshold)
            optimizedTempThresh = app.OptimizedTemperatureThreshold * 100;
        end
        
        if isprop(app, 'QuickSimReactantThreshold') && ~isempty(app.QuickSimReactantThreshold)
            quickSimReactantThresh = app.QuickSimReactantThreshold * 100;
        end
        if isprop(app, 'QuickSimProductThreshold') && ~isempty(app.QuickSimProductThreshold)
            quickSimProductThresh = app.QuickSimProductThreshold * 100;
        end
        if isprop(app, 'QuickSimTemperatureThreshold') && ~isempty(app.QuickSimTemperatureThreshold)
            quickSimTempThresh = app.QuickSimTemperatureThreshold * 100;
        end
        
        % Display convergence thresholds
        info = sprintf('%sCONVERGENCE THRESHOLDS (Relative Change %%):\n', info);
        info = sprintf('%sManual Reactants: %.3f%% | Products: %.3f%% | Temperature: %.3f%%\n', ...
                      info, manualReactantThresh, manualProductThresh, manualTempThresh);
        info = sprintf('%sOptimized Reactants: %.3f%% | Products: %.3f%% | Temperature: %.3f%%\n', ...
                      info, optimizedReactantThresh, optimizedProductThresh, optimizedTempThresh);
        info = sprintf('%sQuick Sim Reactants: %.3f%% | Products: %.3f%% | Temperature: %.3f%%\n', ...
                      info, quickSimReactantThresh, quickSimProductThresh, quickSimTempThresh);
        
        % Ultra-precision thresholds if available
        if isprop(app, 'UltraPrecisionResults') && ~isempty(app.UltraPrecisionResults) && ...
           isprop(app, 'UltraReactantThreshold') && ~isempty(app.UltraReactantThreshold)
            ultraReactantThresh = app.UltraReactantThreshold * 100;
            ultraProductThresh = app.UltraProductThreshold * 100;
            ultraTempThresh = app.UltraTemperatureThreshold * 100;
            info = sprintf('%s🏆 Ultra-Precision Reactants: %.4f%% | Products: %.5f%% | Temperature: %.4f%% (STRICT)\n', ...
                          info, ultraReactantThresh, ultraProductThresh, ultraTempThresh);
        end
        
        info = sprintf('%s\n=== CONVERGENCE RESULTS ===\n', info);
        
        % Manual convergence results
        if isprop(app, 'ManualSimResults') && ~isempty(app.ManualSimResults) && isfield(app.ManualSimResults, 't')
            if isprop(app, 'IsManualConverged') && app.IsManualConverged
                info = sprintf('%sMANUAL (Relaxed): ✓ CONVERGED\n', info);
            else
                info = sprintf('%sMANUAL (Relaxed): ⚠ NOT CONVERGED\n', info);
            end
            
            % Manual convergence details
            if isprop(app, 'ManualConvergenceData') && ~isempty(app.ManualConvergenceData)
                if isfield(app.ManualConvergenceData, 'NaOH_converged') && ...
                   isfield(app.ManualConvergenceData, 'NaOH_time') && ...
                   app.ManualConvergenceData.NaOH_converged
                    info = sprintf('%s  - NaOH: %.2f min (%.6f mol/L)\n', info, ...
                                  app.ManualConvergenceData.NaOH_time, app.ManualConvergenceData.NaOH_conc);
                end
                if isfield(app.ManualConvergenceData, 'EtOAc_converged') && ...
                   isfield(app.ManualConvergenceData, 'EtOAc_time') && ...
                   app.ManualConvergenceData.EtOAc_converged
                    info = sprintf('%s  - EtOAc: %.2f min (%.6f mol/L)\n', info, ...
                                  app.ManualConvergenceData.EtOAc_time, app.ManualConvergenceData.EtOAc_conc);
                end
            end
        end
        
        % Optimized convergence results
        if isprop(app, 'SimResults') && ~isempty(app.SimResults) && isfield(app.SimResults, 't')
            if isprop(app, 'IsConverged') && app.IsConverged
                info = sprintf('%sOPTIMIZED (Standard): ✓ CONVERGED\n', info);
            else
                info = sprintf('%sOPTIMIZED (Standard): ⚠ NOT CONVERGED\n', info);
            end
            
            % Optimized convergence details
            if isprop(app, 'ConvergenceData') && ~isempty(app.ConvergenceData)
                if isfield(app.ConvergenceData, 'NaOH_converged') && ...
                   isfield(app.ConvergenceData, 'NaOH_time') && ...
                   app.ConvergenceData.NaOH_converged
                    info = sprintf('%s  - NaOH: %.2f min (%.6f mol/L)\n', info, ...
                                  app.ConvergenceData.NaOH_time, app.ConvergenceData.NaOH_conc);
                end
                if isfield(app.ConvergenceData, 'EtOAc_converged') && ...
                   isfield(app.ConvergenceData, 'EtOAc_time') && ...
                   app.ConvergenceData.EtOAc_converged
                    info = sprintf('%s  - EtOAc: %.2f min (%.6f mol/L)\n', info, ...
                                  app.ConvergenceData.EtOAc_time, app.ConvergenceData.EtOAc_conc);
                end
            end
        end
        
        % Ultra-precision convergence results
        if isprop(app, 'UltraPrecisionResults') && ~isempty(app.UltraPrecisionResults) && isfield(app.UltraPrecisionResults, 't')
            if isprop(app, 'IsUltraConverged') && app.IsUltraConverged
                info = sprintf('%s🏆 ULTRA-PRECISION (Strict): ✓ CONVERGED\n', info);
            else
                info = sprintf('%s🏆 ULTRA-PRECISION (Strict): ⚠ NOT CONVERGED\n', info);
            end
            
            % Ultra-precision convergence details
            if isprop(app, 'UltraConvergenceData') && ~isempty(app.UltraConvergenceData)
                if isfield(app.UltraConvergenceData, 'NaOH_converged') && ...
                   isfield(app.UltraConvergenceData, 'NaOH_time') && ...
                   app.UltraConvergenceData.NaOH_converged
                    info = sprintf('%s  - NaOH: %.2f min (%.8f mol/L)\n', info, ...
                                  app.UltraConvergenceData.NaOH_time, app.UltraConvergenceData.NaOH_conc);
                end
                if isfield(app.UltraConvergenceData, 'EtOAc_converged') && ...
                   isfield(app.UltraConvergenceData, 'EtOAc_time') && ...
                   app.UltraConvergenceData.EtOAc_converged
                    info = sprintf('%s  - EtOAc: %.2f min (%.8f mol/L)\n', info, ...
                                  app.UltraConvergenceData.EtOAc_time, app.UltraConvergenceData.EtOAc_conc);
                end
            end
        end
        
        % Quick sim convergence results
        if isprop(app, 'QuickSimResults') && ~isempty(app.QuickSimResults) && isfield(app.QuickSimResults, 't')
            if isprop(app, 'IsQuickSimConverged') && app.IsQuickSimConverged
        ...
        if (isprop(app, 'SimResults') && ~isempty(app.SimResults)) || ...
           (isprop(app, 'ManualSimResults') && ~isempty(app.ManualSimResults))
            info = sprintf('%s\n=== PERFORMANCE COMPARISON ===\n', info);
            
            if isprop(app, 'RSquaredManual') && ~isnan(app.RSquaredManual)
                info = sprintf('%sManual R²: %.6f\n', info, app.RSquaredManual);
            end
            if isprop(app, 'RSquared') && ~isnan(app.RSquared)
                info = sprintf('%sOptimized R²: %.6f\n', info, app.RSquared);
            end
            if isprop(app, 'RSquaredUltra') && ~isnan(app.RSquaredUltra)
                info = sprintf('%s🏆 Ultra-Precision R²: %.8f\n', info, app.RSquaredUltra);
            end
            
            % Error comparison
            if isprop(app, 'ComparisonTableData') && ~isempty(app.ComparisonTableData)
                try
                    opt_max_error = max(app.ComparisonTableData.Optimized_Error_percent);
                    info = sprintf('%sOptimized Max Error: %.4f%%\n', info, opt_max_error);
                catch
                    % Handle case where error column doesn't exist
                end
            end
            
            if isprop(app, 'ComparisonTableDataUltra') && ~isempty(app.ComparisonTableDataUltra)
                try
        ...
        info = sprintf('%s\n=== RECOMMENDATIONS ===\n', info);
        if isprop(app, 'UltraPrecisionResults') && ~isempty(app.UltraPrecisionResults) && ...
           isprop(app, 'IsUltraConverged') && app.IsUltraConverged
            info = sprintf('%s🏆 Ultra-precision achieved strict convergence!\n', info);
            info = sprintf('%s✨ Highest accuracy simulation available.\n', info);
        elseif isprop(app, 'SimResults') && ~isempty(app.SimResults) && ...
               isprop(app, 'IsConverged') && app.IsConverged
        ...
        if isprop(app, 'updateComparisonTableDisplay')
            app.updateComparisonTableDisplay();
        end
        
        % Update profile tables
        if isprop(app, 'updateProfileTables')
            app.updateProfileTables();
        end
        
    catch ME
        % Enhanced error handling
        errorInfo = sprintf(['CONVERGENCE ANALYSIS ERROR\n\n' ...
                           'Error: %s\n\n' ...
                           'Basic Status:\n' ...
                           'Manual Simulation: %s\n' ...
                           'Optimized Simulation: %s\n' ...
                           'Data Points: %d\n\n' ...
                           'Debug Info:\n' ...
                           'Error in: %s\n' ...
                           'Line: %s'], ...
                           ME.message, ...
                           iif(isprop(app, 'ManualSimResults') && ~isempty(app.ManualSimResults), 'Available', 'Not Available'), ...
                           iif(isprop(app, 'SimResults') && ~isempty(app.SimResults), 'Available', 'Not Available'), ...
                           iif(isprop(app, 'Data') && ~isempty(app.Data), height(app.Data), 0), ...
                           ME.stack(1).name, ...
                           num2str(ME.stack(1).line));
        
        app.ConvergenceText.Value = errorInfo;
        
        % Log error details
        fprintf('Analysis Display Error: %s\n', ME.message);
        fprintf('Full error report: %s\n', ME.getReport());
    end
end
...
app.ValidateDataButton.ButtonPushedFcn = createCallbackFcn(app, @ValidateDataButtonPushed, true);
         ...
         app.ActionsGrid.RowHeight = {'fit', 'fit', 'fit', 'fit', 'fit', 'fit'};
            ...
            % ADD AFTER app.ClearAllButton creation:
app.UncertaintyButton = uibutton(app.ActionsGrid, 'push');
app.UncertaintyButton.Layout.Row = 5;
app.UncertaintyButton.Layout.Column = 1;
app.UncertaintyButton.Text = '📊 Parameter Uncertainty';
app.UncertaintyButton.FontWeight = 'bold';
app.UncertaintyButton.ButtonPushedFcn = createCallbackFcn(app, @UncertaintyButtonPushed, true);

app.ResidualsButton = uibutton(app.ActionsGrid, 'push');
app.ResidualsButton.Layout.Row = 6;
app.ResidualsButton.Layout.Column = 1;
app.ResidualsButton.Text = '📈 Residuals Analysis';
app.ResidualsButton.FontWeight = 'bold';
app.ResidualsButton.ButtonPushedFcn = createCallbackFcn(app, @ResidualsButtonPushed, true);
...
function UploadButtonPushed(app, ~)
    try
        ...
        % NEW: Enhanced button callback functions (ADD THESE)
function UncertaintyButtonPushed(app, ~)
    if isnan(app.k_fit) || isempty(app.Data)
        app.updateStatus('Need optimized parameters and experimental data', 'warning');
        uialert(app.UIFigure, 'Please run optimization first and ensure experimental data is loaded.', 'Missing Requirements');
        return;
    end
    app.calculateParameterUncertainty();
end

function ResidualsButtonPushed(app, ~)
    if isempty(app.SimResults) || isempty(app.Data)
        app.updateStatus('Need simulation results and experimental data', 'warning');
        uialert(app.UIFigure, 'Please run simulation first and ensure experimental data is loaded.', 'Missing Requirements');
        return;
    end
    app.performResidualsAnalysis();
end
...
app.TotalCoolingRequiredHistory = [];
   ...
   function showCoolingSystemReport(app, temp_deviation, cooling_status)
    ...
    methods (Access = public)
        ...
        % Construct app
        function app = CSTR_Saponification_Modern_needfix
